...
class _EstimateListScreenState extends State<EstimateListScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: _mainAppBar(),
        body: estimates.isEmpty ? _column() : _scrollbar(),
  ...
  Widget _popupMenuButton() {
    return PopupMenuButton(
        ...
        onSelected: (dynamic value) {
          if (value == 1) {
            setState(() {
              estimates.sort((a, b) => a.createdAt!.compareTo(b.createdAt!));
            });
          } else if (value == 2) {
            setState(() {
  ...
  AppBar _mainAppBar() {
    return AppBar(
      ...
      title: RichText(text: _textSpan()),
      actions: estimates.isEmpty ? null : <Widget>[_popupMenuButton()],
    );
  }
    ...
    return ListTile(
      ...
      onTap: () {
        Navigator.pushNamed(context, EstimateScreen.routeName,
                arguments: EstimateScreenArgs(false, estimates[index]))
            .then((value) {
          loadEstimates();
        });
      },
